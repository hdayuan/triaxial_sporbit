...
if __name__=="__main__":
    ...
    fps = ["./3bd_20i_4otn_1e-3j2_100Q_0.025dt/trial_"+str(i)+".txt" for i in 
    range(n_trials)]
    ...
    for i in range(n_trials):
        ...
        if i == 24: # only for 3bd_20i_4otn_1e-3j2_100Q_0.025dt
            continue
        ...
        if np.any(omegas[i] > 8.):
            n_errs += 1
            continue

        ax1.plot(ts[i],omegas[i], lw=1., color='black', alpha=0.2)
        ax2.plot(ts[i],thetas[i], lw=1., color='black', alpha=0.2)
        ax3.plot(ts[i],np.degrees(np.unwrap(np.radians(phis[i]))), lw=1., color='black', alpha=0.2)
        if not together:
